<template>
  <div class="relative min-h-[60vh] w-full flex flex-col items-center justify-center">
    <!-- Fond dégradé identique à l'accueil -->
    <div class="absolute inset-0 -z-10 bg-gradient-to-br from-indigo-200 via-purple-200 to-pink-200"></div>
    <div class="z-10 w-full max-w-2xl mx-auto bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl p-8 sm:p-10 flex flex-col items-center border border-purple-100">
      ...
      <div class="mb-2 flex items-center gap-4">
        ...
        <button @click="resetBoard" class="px-4 py-2 rounded-lg bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold shadow hover:from-indigo-500 hover:to-pink-400 hover:scale-105 transition-all">Recommencer</button>
      </div>
      ...
      <div class="grid grid-cols-4 gap-4 mt-4">
        <div v-for="(card, idx) in cards" :key="card.id" class="relative">
          <button
            ...
            :class="{ 'bg-white border-green-400 scale-105 ring-2 ring-green-300': card.flipped || card.matched, 'opacity-60 pointer-events-none': card.matched }"
            @click="flipCard(idx)"
      ...
      <transition name="fade-bounce">
        <div v-if="isWin" class="mt-8 flex flex-col items-center">
          ...
          <div class="text-green-600 text-2xl font-bold text-center">Bravo !<br>Tu as trouvé toutes les paires</div>
        </div>
      </transition>
    </div>
  </div>
...
function flipCard(idx: number) {
  if (flippedIndexes.value.length === 2 || cards.value[idx].flipped || cards.value[idx].matched) return
...
function resetBoard() {
  cards.value = shuffle([...emojis, ...emojis]).map((emoji, i) => ({
    id: i + '-' + emoji,
    emoji,
    flipped: false,
    matched: false
...
<style scoped>
.card-3d {
  box-shadow: 0 4px 16px 0 rgba(168,139,250,0.15), 0 1.5px 6px 0 rgba(244,114,182,0.10);
  transition: box-shadow 0.2s, transform 0.2s;
}
.card-3d:hover:not(:disabled) {
  box-shadow: 0 8px 32px 0 rgba(168,139,250,0.25), 0 3px 12px 0 rgba(244,114,182,0.18);
